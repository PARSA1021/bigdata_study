...
// 3초 판단 치트키 플래시카드 마스터 딕셔너리 (초빈출 80선 실전 키워드)
const THREE_SEC_CHEATS = [
  // --- 1과목: 기획 & 거버넌스 & 비식별 ---
  { keyword: "각 집단에서 무작위로 표본 추출", concept: "층화표본추출", stage: 1, hint: "집단 내 동질, 집단 간 이질" },
  { keyword: "집단 자체를 무작위로 통째 선택하여 전수조사", concept: "군집표본추출", stage: 1, hint: "집단 내 이질, 집단 간 동질" },
  { keyword: "첫 번째만 랜덤, 이후 일정한 간격 k번째 추출", concept: "계통표본추출", stage: 1, hint: "모집단 주기성 시 편향 주의" },
  { keyword: "모든 개체가 동일한 확률로 순수 난수 추출", concept: "단순무작위추출", stage: 1, hint: "표본추출틀 필수" },
  { keyword: "분석 대상(What) O, 분석 방법(How) O", concept: "최적화 (Optimization)", stage: 1, hint: "기획 4대 유형" },
  { keyword: "분석 대상(What) O, 분석 방법(How) X", concept: "솔루션 (Solution)", stage: 1, hint: "기획 4대 유형" },
  { keyword: "분석 대상(What) X, 분석 방법(How) O", concept: "통찰 (Insight)", stage: 1, hint: "기획 4대 유형" },
  { keyword: "분석 대상(What) X, 분석 방법(How) X", concept: "발견 (Discovery)", stage: 1, hint: "기획 4대 유형" },
  { keyword: "업무이해 ➔ 데이터이해 ➔ 데이터준비 ➔ 모델링 ➔ 평가 ➔ 전개", concept: "CRISP-DM (6단계)", stage: 1, hint: "준비와 모델링 간 피드백" },
  { keyword: "데이터선택 ➔ 전처리 ➔ 변환 ➔ 마이닝 ➔ 평가", concept: "KDD (5단계)", stage: 1, hint: "지식발견 프로세스" },
  { keyword: "문제 탐색 ➔ 문제 정의 ➔ 해결방안 도출 ➔ 타당성 검토", concept: "하향식 접근법 (Top-down)", stage: 1, hint: "Problem First" },
  { keyword: "데이터 탐색 ➔ 가치 발굴 (상향식) + 수렴 결합", concept: "디자인 씽킹 (Design Thinking)", stage: 1, hint: "발산과 수렴의 반복" },
  { keyword: "동일한 준식별자 레코드가 최소 K개 이상 존재", concept: "K-익명성", stage: 2, hint: "연계 공격 방어" },
  { keyword: "민감 정보가 최소 L개 이상 다양하게 분포", concept: "L-다양성", stage: 2, hint: "동질성 / 배경지식 공격 방어" },
  { keyword: "민감 정보의 분포가 전체 분포와 T 이하 차이", concept: "T-근접성", stage: 2, hint: "쏠림 / 왜곡 공격 방어" },
  { keyword: "추가 정보 결합 없이는 식별 불가, 통계/연구 활용 허용", concept: "가명정보", stage: 2, hint: "개인정보보호법 적용 대상" },
  { keyword: "시간/비용/기술을 고려해도 영구 식별 불가", concept: "익명정보", stage: 2, hint: "개인정보보호법 적용 제외" },
  { keyword: "전사 분석 전담조직이 각 현업 부서에 인력 배치 협업", concept: "분산형 조직 구조", stage: 1, hint: "집중형 vs 기능형 vs 분산형" },

  // --- 2과목: 탐색 & 전처리 & 통계 ---
  { keyword: "소득 미응답처럼 결측 이유 자체가 해당 값과 연관", concept: "비무작위 결측 (MNAR)", stage: 2, hint: "삭제 시 심각한 편향 발생" },
  { keyword: "결측이 다른 모든 변수 및 값과 무관", concept: "완전 무작위 결측 (MCAR)", stage: 2, hint: "단순 삭제 가능" },
  { keyword: "결측이 다른 관측 변수와 통계적으로 연관", concept: "무작위 결측 (MAR)", stage: 2, hint: "대치법 활용" },
  { keyword: "IQR = Q3 - Q1, 하한 Q1 - 1.5*IQR, 상한 Q3 + 1.5*IQR", concept: "IQR 이상치 판별식", stage: 2, hint: "박스플롯 수염 경계" },
  { keyword: "z = (X - μ) / σ, |z| >= 3 이면 이상치 판정", concept: "Z-Score 3시그마 법칙", stage: 2, hint: "정규분포 기준 99.7% 밖" },
  { keyword: "x_norm = (x - Min) / (Max - Min) (0~1 변환)", concept: "Min-Max 정규화", stage: 3, hint: "이상치에 매우 민감" },
  { keyword: "비정규 분포를 정규분포로 변환하는 거듭제곱 모수 탐색", concept: "Box-Cox 변환", stage: 3, hint: "등분산성/정규성 확보" },
  { keyword: "평균 > 중앙값 > 최빈값 (오른쪽으로 긴 꼬리)", concept: "양의 왜도 (Skewness > 0)", stage: 3, hint: "평 > 중 > 최" },
  { keyword: "최빈값 > 중앙값 > 평균 (왼쪽으로 긴 꼬리)", concept: "음의 왜도 (Skewness < 0)", stage: 3, hint: "최 > 중 > 평" },
  { keyword: "p-value < 유의수준 0.05", concept: "귀무가설(H₀) 기각 ➔ 대립가설(H₁) 채택", stage: 4, hint: "통계적으로 유의미한 차이 있음" },
  { keyword: "귀무가설이 참인데 잘못 기각한 위양성 오류", concept: "1종 오류 (α)", stage: 4, hint: "더 치명적인 오류" },
  { keyword: "대립가설이 참인데 올바르게 H₀를 기각할 확률", concept: "검정력 (Power = 1 - β)", stage: 4, hint: "2종 오류의 여사건" },
  { keyword: "연속형 변수 간의 선형적 상관성 측정 (이상치 민감)", concept: "피어슨 상관계수 (Pearson)", stage: 5, hint: "모수적 상관분석" },
  { keyword: "순위 척도 데이터 간의 단조적 상관성 측정", concept: "스피어만 상관계수 (Spearman)", stage: 5, hint: "비모수적 상관분석" },
  { keyword: "3개 이상 집단의 평균 차이 유의성 비교 (F-검정)", concept: "분산분석 (ANOVA)", stage: 4, hint: "집단간분산 / 집단내분산" },
  { keyword: "두 범주형 변수 간의 독립성/관련성 검정 (χ²)", concept: "카이제곱 독립성 검정", stage: 4, hint: "(관측-기대)² / 기대" },
  { keyword: "소표본(n < 2000) 대표 정규성 검정", concept: "Shapiro-Wilk 검정", stage: 3, hint: "Q-Q 플롯과 병행" },
  { keyword: "점들이 대각선 45도 직선에 정렬되면 정규성 만족", concept: "Q-Q Plot", stage: 3, hint: "시각적 정규성 검토" },
  { keyword: "서로 독립인 직교 축으로 차원 축소 (분산 최대화)", concept: "주성분분석 (PCA)", stage: 5, hint: "제1주성분 분산 최대" },
  { keyword: "고차원 비선형 거리를 2~3차원 시각화로 압축", concept: "t-SNE", stage: 5, hint: "시각화 전용 매핑" },
  { keyword: "VIF = 1 / (1 - R²) >= 10", concept: "다중공선성 (Multicollinearity)", stage: 6, hint: "PCA/릿지로 해결" },

  // --- 3과목: 모델링 & 머신러닝 ---
  { keyword: "회귀 잔차의 독립성 검정 통계량 (2에 근접 시 독립)", concept: "더빈-왓슨 (Durbin-Watson)", stage: 6, hint: "시계열 잔차 자기상관 검정" },
  { keyword: "L1 규제, 가중치 절댓값 페널티 ➔ 계수를 '정확히 0'으로", concept: "Lasso (라쏘) 회귀", stage: 7, hint: "변수 선택 효과 (Sparse)" },
  { keyword: "L2 규제, 가중치 제곱합 페널티 ➔ 계수를 '0에 가깝게 축소'", concept: "Ridge (릿지) 회귀", stage: 7, hint: "다중공선성 완화" },
  { keyword: "로지스틱 회귀 독립변수 1 증가 시 오즈(Odds) 증가 배수", concept: "e의 β₁제곱 배 (exp(β₁))", stage: 8, hint: "로짓 변환 지수승" },
  { keyword: "이진 분류 다중 분기 시 확률 총합 1.0 정규화", concept: "소프트맥스 (Softmax)", stage: 8, hint: "출력층 활성화 함수" },
  { keyword: "CART 알고리즘의 분류 분할 기준 불순도", concept: "지니 지수 (Gini Index)", stage: 9, hint: "1 - Σ(pᵢ²)" },
  { keyword: "C4.5 / C5.0 알고리즘의 분할 기준", concept: "정보획득률 (Information Gain Ratio)", stage: 9, hint: "엔트로피 기반" },
  { keyword: "과적합 방지를 위해 불필요한 마디를 잘라내는 작업", concept: "가지치기 (Pruning)", stage: 9, hint: "사전/사후 가지치기" },
  { keyword: "복원추출(부트스트랩) 병렬 앙상블 ➔ '분산(Variance) 감소'", concept: "배깅 / Random Forest", stage: 9, hint: "과적합 방지 특화" },
  { keyword: "오차에 가중치를 부여하는 순차 앙상블 ➔ '편향(Bias) 감소'", concept: "부스팅 (XGBoost, LightGBM)", stage: 9, hint: "성능 극대화 특화" },
  { keyword: "부트스트랩 시 미추출되는 약 36.8%의 검증용 데이터", concept: "Out-of-Bag (OOB)", stage: 9, hint: "자체 일반화 검증" },
  { keyword: "기본 모델들의 예측 결과를 메타 특성으로 재학습", concept: "스태킹 (Stacking)", stage: 9, hint: "메타 러너" },
  { keyword: "결정 초평면과 서포트 벡터 사이 마진(Margin) 최대화", concept: "SVM (서포트 벡터 머신)", stage: 10, hint: "최대 마진 분류기" },
  { keyword: "저차원 비선형 공간을 고차원 선형 공간으로 매핑", concept: "커널 트릭 (Kernel Trick)", stage: 10, hint: "RBF, 다항식 커널" },
  { keyword: "SVM에서 하드마진(오분류 불허, 과적합 위험)을 결정하는 값", concept: "C (Cost) 파라미터", stage: 10, hint: "C 클수록 하드마진" },
  { keyword: "거리 기반, 사전 군집 수 K 지정 필수, 이상치에 민감", concept: "K-Means 군집", stage: 11, hint: "엘보우 기법으로 K 결정" },
  { keyword: "밀도 기반, 사전 K 불필요, 노이즈(이상치) 자동 분리", concept: "DBSCAN", stage: 11, hint: "Eps, minPts 파라미터" },
  { keyword: "계층적 군집 중 '군집 내 오차제곱합(SSE) 증가량 최소화'", concept: "와드 연결법 (Ward's Method)", stage: 11, hint: "가장 안정적인 계층 군집" },
  { keyword: "비지도 신경망, 경쟁층 승자독식(Winner-Takes-All)", concept: "SOM (자가조직지도)", stage: 11, hint: "2차원 격자 위상 매핑" },
  { keyword: "양수 영역 기울기 1로 은닉층 기울기 소실(Vanishing) 완벽 해결", concept: "ReLU 활성화 함수", stage: 15, hint: "max(0, x)" },
  { keyword: "학습 시 뉴런을 무작위로 비활성화하여 과적합 방지", concept: "드롭아웃 (Dropout)", stage: 15, hint: "정규화 기법" },
  { keyword: "검증 손실(Validation Loss)이 증가하기 시작할 때 학습 중단", concept: "조기 종료 (Early Stopping)", stage: 15, hint: "과적합 방지" },
  { keyword: "학습 파라미터(가중치)가 0개인 특징 맵 다운샘플링 계층", concept: "풀링 계층 (Max/Avg Pooling)", stage: 15, hint: "CNN 연산량 축소" },
  { keyword: "과거 기억을 얼마나 지울지 결정하는 LSTM 게이트", concept: "망각 게이트 (Forget Gate)", stage: 15, hint: "Cell State 장기기억" },
  { keyword: "앞으로 이동할 위치의 기울기를 미리 확인하여 제동", concept: "NAG (Nesterov Accelerated Gradient)", stage: 15, hint: "모멘텀 오버슈팅 방지" },
  { keyword: "모멘텀(방향) + RMSprop(보폭)을 결합한 표준 옵티마이저", concept: "Adam (Adaptive Moment)", stage: 15, hint: "가장 대중적" },
  { keyword: "RNN 없는 완전 병렬 Self-Attention 모델", concept: "트랜스포머 (Transformer)", stage: 16, hint: "Attention Is All You Need" },

  // --- 4과목: 결과해석 & 평가 지표 ---
  { keyword: "예측 양성(TP+FP) 중 실제 양성(TP) 비율 (스팸/유죄 중시)", concept: "정밀도 (Precision / PPV)", stage: 4, hint: "1종 오류(FP) 최소화" },
  { keyword: "실제 양성(TP+FN) 중 맞힌 양성(TP) 비율 (암/화재/불량 중시)", concept: "재현율 (Recall / 민감도 / TPR)", stage: 4, hint: "2종 오류(FN) 최소화" },
  { keyword: "실제 음성(TN+FP) 중 맞힌 음성(TN) 비율", concept: "특이도 (Specificity / TNR)", stage: 4, hint: "1 - FPR" },
  { keyword: "정밀도(Precision)와 재현율(Recall)의 조화평균", concept: "F1-Score", stage: 4, hint: "2*(P*R)/(P+R)" },
  { keyword: "ROC 곡선의 X축 지표 (1 - 특이도)", concept: "FPR (위양성률 = FP / (TN + FP))", stage: 4, hint: "가로축" },
  { keyword: "ROC 곡선의 Y축 지표 (민감도)", concept: "TPR (재현율 = TP / (TP + FN))", stage: 4, hint: "세로축" },
  { keyword: "모델 적용 시 무작위 대비 반응률 배수 (기준선 1.0)", concept: "향상도 (Lift)", stage: 14, hint: "Lift > 1 양의 상관" },
  { keyword: "회귀 지표 중 큰 이상치에 민감하게 페널티 부과", concept: "RMSE / MSE", stage: 4, hint: "오차 제곱 평균" },
  { keyword: "회귀 지표 중 오차 절댓값으로 이상치에 로버스트", concept: "MAE", stage: 4, hint: "절댓값 오차" },
  { keyword: "군집 응집도와 분리도 측정, -1에서 +1 사이 값", concept: "실루엣 계수 (Silhouette)", stage: 11, hint: "+1 최상, -1 오분류" },
  { keyword: "타겟 클래스 불균형 시 각 Fold의 클래스 비율 유지 분할", concept: "층화 K-Fold (Stratified K-Fold)", stage: 4, hint: "불균형 교차검증 필수" },
  { keyword: "KNN 이웃 간 선분 보간으로 가상 소수 샘플 합성 생성", concept: "SMOTE 오버샘플링", stage: 13, hint: "단순 복제 과적합 방지" },
  { keyword: "A와 B가 동시에 일어날 전체 확률 P(A∩B)", concept: "지지도 (Support)", stage: 14, hint: "최소 지지도 기준" },
